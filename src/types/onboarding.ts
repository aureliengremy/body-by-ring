...
export interface OnboardingData {
  ...
  // Fitness Assessment
  can_do_pushups: number; // Max pushups in a row
  can_do_pullups: number; // Max pullups in a row
  can_hold_plank: number; // Plank hold in seconds
...
export function calculateExperienceLevel(
  ...
  const pushupScore =
    pushups <= LEVEL_CRITERIA.beginner.pushups.max
      ? 0
      : pushups <= LEVEL_CRITERIA.intermediate.pushups.max
      ? 1
      : 2;

  const pullupScore =
    pullups <= LEVEL_CRITERIA.beginner.pullups.max
      ? 0
      : pullups <= LEVEL_CRITERIA.intermediate.pullups.max
      ? 1
      : 2;

  const plankScore =
    plankSeconds <= LEVEL_CRITERIA.beginner.plank.max
      ? 0
      : plankSeconds <= LEVEL_CRITERIA.intermediate.plank.max
      ? 1
      : 2;
