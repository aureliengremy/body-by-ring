  ...
  useEffect(() => {
    async function loadWorkouts() {
      ...
      try {
        ...
        if (!program) {
          setError('No active program found. Please complete onboarding first.')
          return
        }
        ...
        const workoutsWithDetails: WorkoutWithDetails[] = workoutData?.map(workout => {
            ...
            {(['all', 'pending', 'completed'] as const).map((filterType) => (
              ...
              >
                ...
                <span className="ml-2 text-xs bg-gray-200 px-2 py-1 rounded-full">
                  {workouts.filter(w => 
                    filterType === 'all' ? true :
                    filterType === 'completed' ? w.completed_at :
                    !w.completed_at
                  ).length}
                </span>
              </button>
        ...
        {filteredWorkouts.length === 0 ? (
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-2">
              {filter === 'completed' ? 'No completed workouts yet' : 'No workouts available'}
            </h3>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {filteredWorkouts.map(workout => (
              <Card 
                key={workout.id} 
                className={`${workout.completed_at ? 'bg-green-50 border-green-200' : 'hover:shadow-lg'} transition-shadow`}
              >
                <CardHeader>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="space-y-1">
                        {workout.exercises.slice(0, 3).map(exercise => {
                          const categoryInfo = CATEGORY_INFO[exercise.category as keyof typeof CATEGORY_INFO]
                    ...
                    <div className="flex gap-2 pt-4">
                      {workout.completed_at ? (
                        <>
                          ...
                          <Button 
                            variant="outline"
                            onClick={() => router.push(`/workouts/${workout.id}/review`)}
                          >
                            Review
                          </Button>
                        </>
                        ...
                        <>
                          ...
                          <Button 
                            variant="outline"
                            onClick={() => router.push(`/workouts/${workout.id}/preview`)}
                          >
                            Preview
                          </Button>
                        </>
